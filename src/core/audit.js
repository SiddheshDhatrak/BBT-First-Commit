...
function createAuditService(repo) {
  function append({ entityType, entityId, action, actorId, payload = {} }) {
    ...
    const event = { id: randomUUID(), entityType, entityId, action, actorId, payload, occurredAt: new Date().toISOString() };
    const prevHash = previous?.rowHash || 'GENESIS';
    return repo.insert('auditLogs', { ...event, prevHash, rowHash: sha256(`${canonical(event)}|${prevHash}`) });
  }
  function verify() {
    ...
    for (const event of repo.list('auditLogs')) {
      ...
      if (prevHash !== expectedPrevHash || rowHash !== expectedHash) return { valid: false, firstBrokenRecordId: event.id };
