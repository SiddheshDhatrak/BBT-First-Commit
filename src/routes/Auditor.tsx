...
export function InvestigationQueue() {
  ...
  return (
    <div>
      <PageHeader eyebrow="Auditor console" title="Investigation Queue" sub="Sorted by risk score. Every row shows a badge plus one-line evidence." action={<LiveBadge live={liveOk} />} />
      <SignalBanner />
      {liveOk && (
        <Reveal className="mt-4">
      ...
      <Reveal>
        <div className="rs-card flex max-h-[60vh] min-h-[340px] flex-col overflow-hidden">
          <div className="flex items-center gap-3 border-b px-5 py-3.5" style={{ borderColor: "var(--border-subtle)" }}>
            ...
            <div>
              <p className="flex items-center gap-2 text-[14px] font-extrabold">Copilot · ledger-grounded <LiveBadge live={isAgentEnabled()} label="AGENT LIVE" /></p>
