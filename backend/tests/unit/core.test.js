...
async function runUnitTests() {
  ...
  const hdist = hammingDistance(
    'dhash:0101010101010101010101010101010101010101010101010101010101010101',
    'dhash:0101010101010101010101010101010101010101010101010101010101010101'
  );
  assert(hdist === 0, 'hammingDistance: identical hashes have distance 0');
  const hdist2 = hammingDistance(
  ...
  try {
    relief.createExpense(
      { invoiceId: 'invalid', budgetCategoryId: 'budget-category-food' },
      ngoActor
    );
  ...
  try {
    delivery.createBeneficiary(
      { programId: PROGRAM_ID, householdHash: 'sha256:h1', district: 'Dhubri' },
      { ...ngoActor, organizationId: 'other-org' }
    );
  ...
  const ben2 = delivery.createBeneficiary(
    {
      programId: PROGRAM_ID,
      householdHash: 'sha256:h2',
      phoneHash: 'sha256:p2',
      district: 'Dhubri',
    },
    ngoActor
  );
  const ben3 = delivery.createBeneficiary(
    {
      programId: PROGRAM_ID,
      householdHash: 'sha256:h1',
      phoneHash: 'sha256:p1',
  ...
  const distObj1 = delivery.createDistribution(
    { expenseId: expense.id, beneficiaryId: ben1.id, quantity: 300 },
    ngoActor
  );
  const distObj2 = delivery.createDistribution(
    { expenseId: expense.id, beneficiaryId: ben2.id, quantity: 300 },
    ngoActor
  );
  const distObj3 = delivery.createDistribution(
    { expenseId: expense.id, beneficiaryId: ben3.id, quantity: 300 },
    ngoActor
  );
  ...
  assert(
    proof1.checks.some(c => c.checkType === 'GEOFENCE' && c.result === 'FAIL'),
    'POD-002: GEOFENCE check fails'
  );
  ...
  assert(
    proof2.checks.some(c => c.checkType === 'PHOTO_HASH' && c.result === 'FAIL'),
    'POD-004: Duplicate photo hash flagged'
  );
  assert(
    proof2.checks.some(c => c.checkType === 'PHOTO_SIMILARITY' && c.result === 'FAIL'),
    'POD-004: Perceptual hash similarity flagged'
  );

  const proof3 = await delivery.uploadProof(
    distObj3.id,
    {
      fileKey: 'proof3.jpg',
      photoHash: 'phash:def456',
      gpsLat: 26.0,
      gpsLng: 90.0,
      capturedAt: new Date().toISOString(),
    },
  ...
  assert(
    allAlerts.some(a => a.evidence.ruleId === 'POD-006'),
    'POD-006: Beneficiary deduplication alert created'
  );
  ...
  assert(
    ['HIGH', 'MEDIUM', 'LOW', 'VERY_LOW', 'NONE'].includes(deliveryConfidence.level),
    'Delivery confidence: valid level'
  );
  assert(
    typeof deliveryConfidence.passRate === 'number',
    'Delivery confidence: passRate is number'
  );

  console.log('\n5. State Transition Tests');
  assert(
    payment.expense.status === 'DELIVERY_PENDING',
    'Payment: Returned expense has DELIVERY_PENDING'
  );
  // Live expense may be FLAGGED after proof uploads; payment return value is authoritative

  console.log('\n6. Idempotency Test');
  const payment2 = relief.payExpense(
    { expenseId: expense.id, toAccountLast4: '1234' },
    ngoActor,
    'test-payment'
  );
  ...
  const result = delivery.recordAuditResult(
    audits[0].id,
    { result: 'CONFIRMED', notes: 'Verified' },
    govtActor
  );
