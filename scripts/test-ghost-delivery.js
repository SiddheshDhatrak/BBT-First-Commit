...
async function runGhostDeliveryTest() {
  ...
  const fieldActor = { id: 'field-test', role: 'FIELD', organizationId: 'org-rahat-demo' };

  try {
    console.log('1. Creating disaster...');
    const disaster = relief.createDisaster({ name: 'Test Flood 2026', type: 'FLOOD', state: 'Assam', district: 'Dhubri', deliveryPolicy: { proofDeadlineHours: 72, randomAuditRate: 0.1, allowedBounds: { minLat: 25.8, maxLat: 26.2, minLng: 89.7, maxLng: 90.1 } } }, govtActor);
    console.log(`   Disaster created: ${disaster.id}\n`);

    console.log('2. Creating campaign...');
    const campaign = relief.createCampaign({ disasterId: disaster.id, name: 'Test Flood Relief', targetAmount: 100000000 });
    console.log(`   Campaign created: ${campaign.id}\n`);

    console.log('3. Recording donation...');
    const donation = relief.donate(campaign.id, { amount: 5000, method: 'SIMULATED_UPI' }, donorActor);
    console.log(`   Donation recorded: ${donation.id} (₹${donation.amount})\n`);

    console.log('4. Allocating funds...');
    const allocation = relief.allocateFunds({ campaignId: campaign.id, organizationId: ngoActor.organizationId, amount: 20000000 }, govtActor);
    console.log(`   Allocation approved: ${allocation.id} (₹${allocation.amount})\n`);

    console.log('5. Creating vendor...');
    const vendor = relief.createVendor({ name: 'Test Food Supplies', gstin: 'TEST-GSTIN-001' }, ngoActor);
    console.log(`   Vendor created: ${vendor.id}\n`);

    console.log('6. Adding vendor bank account...');
    const bankAccount = relief.addVendorBankAccount(vendor.id, { accountNumberHash: 'hash:acc-12345', ifsc: 'TEST0001234' }, ngoActor);
    ...
    const invoice = relief.uploadInvoice({ purchaseOrderId: po.id, invoiceNumber: 'TEST-850K-001', amount: 850000, fileKey: 'synthetic/invoices/TEST-850K-001.pdf', invoiceHash: 'invoice-clean-test' }, ngoActor);
    ...
    const payment = relief.payExpense({ expenseId: expense.id, toAccountLast4: '4242' }, ngoActor, 'test-payment-001');
    ...
    const ben1 = delivery.createBeneficiary({ programId: 'program-food-kits', householdHash: 'sha256:household-1', phoneHash: 'sha256:phone-1', district: 'Dhubri' }, ngoActor);
    const ben2 = delivery.createBeneficiary({ programId: 'program-food-kits', householdHash: 'sha256:household-2', phoneHash: 'sha256:phone-2', district: 'Dhubri' }, ngoActor);
    console.log(`   Beneficiaries created: ${ben1.id}, ${ben2.id}\n`);

    console.log('12. Recording distributions...');
    const dist1 = delivery.createDistribution({ expenseId: expense.id, beneficiaryId: ben1.id, quantity: 600, distributedAt: new Date().toISOString() }, ngoActor);
    const dist2 = delivery.createDistribution({ expenseId: expense.id, beneficiaryId: ben2.id, quantity: 600, distributedAt: new Date().toISOString() }, ngoActor);
    console.log(`   Distributions: ${dist1.id} (${dist1.quantity}), ${dist2.id} (${dist2.quantity})\n`);

    console.log('13. Uploading proof with GPS outside region (Delhi coords)...');
    const proof1 = await delivery.uploadProof(dist1.id, { fileKey: 'synthetic/proofs/test-1.jpg', photoHash: 'phash:staged-photo-001', gpsLat: 28.6139, gpsLng: 77.2090, capturedAt: new Date().toISOString() }, ngoActor);
    ...
    const proof2 = await delivery.uploadProof(dist2.id, { fileKey: 'synthetic/proofs/test-2.jpg', photoHash: 'phash:staged-photo-001', gpsLat: 28.6139, gpsLng: 77.2090, capturedAt: new Date().toISOString() }, ngoActor);
    ...
    alerts.forEach(a => console.log(`   - ${a.evidence.ruleId} (${a.severity}): ${a.evidence.message}`));
    console.log();
    ...
    console.log(`   Expenses with delivery flagged: ${govDashboard.expenses.filter(e => e.delivery.status === 'DELIVERY_FLAGGED').length}`);
    ...
    console.log(`   Risk Score: ${aiResponse.riskScore.total} (${JSON.stringify(aiResponse.riskScore.components)})`);
    console.log(`   Delivery Confidence: ${aiResponse.deliveryConfidence.level} - ${aiResponse.deliveryConfidence.message}\n`);
    ...
    if (audits.length > 0) {
      const auditResult = delivery.recordAuditResult(audits[0].id, { result: 'DISCREPANCY', notes: 'No physical evidence of distribution found at site.' }, govtActor);
    ...
    for (const alert of openAlerts) {
      oversight.resolveAlert(alert.id, { status: 'RESOLVED', reason: 'Verified as ghost delivery through field audit.' }, govtActor);
