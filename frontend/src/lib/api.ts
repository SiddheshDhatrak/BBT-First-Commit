 ...
 * Vendor has no backend role; NGO is closest.
...
export const api = {
  ...
  publicDashboard: <T,>(): Promise<T> => { requireApi(); return fetch(`${BASE}/dashboard/public`).then((r) => { if (!r.ok) throw new Error(`public ${r.status}`); return r.json(); }); },
  ...
  confirmDistribution: <T,>(distributionId: string, body: unknown): Promise<T> => {
    requireApi();
    return fetch(`${BASE}/distributions/${encodeURIComponent(distributionId)}/confirm`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(body),
    }).then(async (r) => {
