...
test('a clean invoice remains delivery-pending until independent evidence is verified', async t => {
  ...
  const demo = await request(baseUrl, '/api/v1/demo/ghost-delivery', {
    method: 'POST',
    headers: governmentHeaders,
    body: '{}',
  });
  assert.equal(demo.status, 200);
  assert.equal(demo.body.invoice.status, 'VERIFIED');
  assert.equal(demo.body.payment.expense.status, 'DELIVERY_PENDING');
  assert.equal(demo.body.proofResults[0].deliveryStatus, 'DELIVERY_FLAGGED');
  ...
  const chain = await request(baseUrl, '/api/v1/audit-chain/verify', {
    headers: governmentHeaders,
  });
